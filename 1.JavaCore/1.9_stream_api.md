...
#### Рецепты
1. Группировка элементов (Collectors.groupingBy). Разбиваем список строк на группы по длине:
```java
List<String> words = List.of("apple", "banana", "cat", "dog", "elephant");
Map<Integer, List<String>> grouped = words.stream()
        .collect(Collectors.groupingBy(String::length));
System.out.println(grouped); // {3=[cat, dog], 5=[apple], 6=[banana], 8=[elephant]}
```
